 ...
 * word/blanks and who's drawing, persistent for the whole turn. This is
 * the ONLY place the word/blanks show at all now — there's no separate
 * reveal overlay on the canvas anymore (the drawer already knows the
 ...
 * room-metadata card is gone — self-contained, no props, so ChatPanel
 * can render it unconditionally and let it decide whether there's
 * anything to show. `rounded-t-2xl` matches the chat card's own corner
 * radius since this sits flush at its top, not floating separately over
 * the canvas the way it used to.
 */
